    ...
    ui(new Ui::DogsList)
{
    ...
    connect(ui->DodajButton, &QPushButton::clicked, this, &DogsList::openAddForm);
}
...
void DogsList::openAddForm()
{
    QDialog *addDialog = new QDialog(this);
    Ui::Dodaj Dodaj;
    Dodaj.setupUi(addDialog);


    addDialog->setWindowFlags(Qt::FramelessWindowHint);


    QPushButton *closeButton = new QPushButton("X", addDialog);
    closeButton->setStyleSheet("QPushButton { color: green; font-weight: bold; }");
    closeButton->setFixedSize(50,50);
    closeButton->setIconSize(QSize(10, 10));
    closeButton->setCursor(Qt::PointingHandCursor);
    closeButton->move(addDialog->width() - closeButton->width() - 10, 10);

    connect(closeButton, &QPushButton::clicked, addDialog, &QDialog::close);
